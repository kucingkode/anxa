> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/medication/
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource Medication yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource Medication tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Medication/:id
```
...
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Medication yang akan dilihat detailnya. |
...
Bila resource Medication dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource Medication yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  //data.terkait.resource.Medication
}
```
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Medication/4a6d884b-8d4b-4e16-b192-6416502d0999"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Medication yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource Medication ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource Medication ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *Medication* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource Medication ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource Medication yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Medication yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource Medication yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Medication yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource Medication ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource Medication yang akan diubah dan juga nama/ID elemen dari Medication yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* Medication dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* Medication dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* Medication ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `Medication.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
