> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/care-plan/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource CarePlan yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/CarePlan
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `subject` | `string` | Berisi ID dari subjek (pasien) yang akan dicari.  Contoh: `{qs-encounter-subject}`. |

##### 2. Pencarian Berdasarkan ID Subjek (Pasien) dan ID *Encounter* (Kunjungan)

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?subject` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *Encounter* (kunjungan)**. Berisi ID dari subjek (pasien) yang akan dicari.  Contoh: `100000000001`. |
| `?encounter` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *Encounter* (kunjungan)**. Berisi ID dari *Encounter* (kunjungan) yang akan dicari.  Contoh: `4f735a03-128b-464d-bf91-e6eacdf1c38f`. |
...
Bila resource CarePlan dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource CarePlan yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/CarePlan/eddbb6b1-6628-49a1-905f-1a01c0673592",
      "resource": {
        "resourceType": "CarePlan",
        "id": "eddbb6b1-6628-49a1-905f-1a01c0673592",
        //data.terkait.resource.CarePlan
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/CarePlan?subject=100000000001"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource CarePlan yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource CarePlan tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/CarePlan/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource CarePlan yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/CarePlan/eddbb6b1-6628-49a1-905f-1a01c0673592"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource CarePlan yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource CarePlan ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource CarePlan ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *CarePlan* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource CarePlan ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource CarePlan yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource CarePlan yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource CarePlan yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource CarePlan yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource CarePlan ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource CarePlan yang akan diubah dan juga nama/ID elemen dari CarePlan yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* CarePlan dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* CarePlan dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* CarePlan ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `CarePlan.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
