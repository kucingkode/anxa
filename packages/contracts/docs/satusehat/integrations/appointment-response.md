> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/appointment-response/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource AppointmentResponse yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/AppointmentResponse
```
...
##### 1. Pencarian Berdasarkan ID *Appointment* (Janji Temu)

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `appointment` | `uuid` | Berisi ID dari *Appointment* (janji temu) yang akan dicari.  Contoh: `98eaf00e-4464-4dfc-a3cc-da9646f99108`. |
...
Bila resource AppointmentResponse dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource AppointmentResponse yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/AppointmentResponse/1bd8d04e-c5c9-4072-90da-05b218b8d26b",
      "resource": {
        "resourceType": "AppointmentResponse",
        "id": "1bd8d04e-c5c9-4072-90da-05b218b8d26b",
        //data.terkait.resource.AppointmentResponse
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/AppointmentResponse?appointment=98eaf00e-4464-4dfc-a3cc-da9646f99108"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource AppointmentResponse yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource AppointmentResponse tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/AppointmentResponse/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource AppointmentResponse yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/AppointmentResponse/1bd8d04e-c5c9-4072-90da-05b218b8d26b"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource AppointmentResponse yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource AppointmentResponse ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource AppointmentResponse ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *AppointmentResponse* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource AppointmentResponse ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource AppointmentResponse yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource AppointmentResponse yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource AppointmentResponse yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource AppointmentResponse yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource AppointmentResponse ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource AppointmentResponse yang akan diubah dan juga nama/ID elemen dari AppointmentResponse yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* AppointmentResponse dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* AppointmentResponse dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* AppointmentResponse ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `AppointmentResponse.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
