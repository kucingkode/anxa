> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/related-person/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource RelatedPerson yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/RelatedPerson
```
...
##### Pencarian Berdasarkan NIK dari *Related Person* (Relasi/ Kerabat Pasien)

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?identifier` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan NIK *Related Person* (Relasi/ Kerabat Pasien)**. Berisi NIK Induk dari *related person* yang akan dicari. Nilai yang dimasukkan harus mempunyai format:  ``` https://fhir.kemkes.go.id/id/nik|<nilai-nik> ```  di mana `<nilai-nik>` berisi NIK *related person* (relasi/ kerabat pasien) yang akan dicari. Sebagai contoh, diketahui NIK relasi/ kerabat pasien `################`, sehingga nilai yang dimasukkan untuk parameter ini adalah:  ``` https://fhir.kemkes.go.id/id/nik|################ ``` |
...
Bila resource RelatedPerson dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource RelatedPerson yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/RelatedPerson/6694e8c8-052a-4ea6-8072-157b6d47ca08",
      "resource": {
        "resourceType": "RelatedPerson",
        "id": "6694e8c8-052a-4ea6-8072-157b6d47ca08",
        //data.terkait.resource.RelatedPerson
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/RelatedPerson?partof={qs-organization-pid}"
```
...
Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource RelatedPerson ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource RelatedPerson ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *RelatedPerson* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource RelatedPerson ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource RelatedPerson yang akan diubah.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/RelatedPerson/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource RelatedPerson yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource RelatedPerson yang sebelumnya telah dikirim.
...
curl --insecure --location ^
  ...
  --request PUT ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/RelatedPerson/6694e8c8-052a-4ea6-8072-157b6d47ca08"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource RelatedPerson yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource RelatedPerson ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource RelatedPerson yang akan diubah dan juga nama/ID elemen dari RelatedPerson yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* RelatedPerson dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* RelatedPerson dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* RelatedPerson ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `RelatedPerson.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
