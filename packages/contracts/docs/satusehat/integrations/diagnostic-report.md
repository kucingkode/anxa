> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/diagnostic-report/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource DiagnosticReport yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/DiagnosticReport
```
...
##### 2. Pencarian Berdasarkan ID Subjek (Pasien) dan ID *Specimen*

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?subject` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *Specimen***. Berisi ID dari subjek (pasien) yang akan dicari.  Contoh: `100000000001`. |
| `?specimen` | `uuid` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *Specimen***. Berisi ID dari *ServiceRequest* yang akan dicari.  Contoh: `5edd0663-093f-40f9-bf04-0c103fd6ec32`. |
...
Bila resource DiagnosticReport dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource DiagnosticReport yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/DiagnosticReport/aaf3f318-00f7-4770-b29b-770bc5bc83a7",
      "resource": {
        "resourceType": "DiagnosticReport",
        "id": "aaf3f318-00f7-4770-b29b-770bc5bc83a7",
        //data.terkait.resource.DiagnosticReport
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/DiagnosticReport?subject=100000000001"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource DiagnosticReport yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource DiagnosticReport tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/DiagnosticReport/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource DiagnosticReport yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/DiagnosticReport/aaf3f318-00f7-4770-b29b-770bc5bc83a7"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource DiagnosticReport yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource DiagnosticReport ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource DiagnosticReport ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *DiagnosticReport* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource DiagnosticReport ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource DiagnosticReport yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource DiagnosticReport yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource DiagnosticReport yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource DiagnosticReport yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource DiagnosticReport ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource DiagnosticReport yang akan diubah dan juga nama/ID elemen dari DiagnosticReport yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* DiagnosticReport dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* DiagnosticReport dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* DiagnosticReport ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `DiagnosticReport.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
