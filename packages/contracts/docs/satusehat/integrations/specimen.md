> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/specimen/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource Specimen yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Specimen
```
...
##### 2. Pencarian Berdasarkan ID Subjek (Pasien) dan ID *Practitioner* (Praktisi/*Collector*)

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?subject` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *Practitioner* (Praktisi/*Collector*)**. Berisi ID dari subjek (pasien) yang akan dicari.  Contoh: `100000000001`. |
| `?collector` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *Practitioner* (Praktisi/*Collector*)**. Berisi ID dari *Practitioner*, bertugas sebagai *collector* (pengumpul) data sampel, yang akan dicari.  Contoh: `N10000001`. |

##### 3. Pencarian Berdasarkan ID Subjek (Pasien) dan Tanggal Pengumpulan

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?subject` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan tanggal pengumpulan**. Berisi ID dari subjek (pasien) yang akan dicari.  Contoh: `100000000001`. |
| `?collected` | `date` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan tanggal pengumpulan**. Berisi tanggal dengan format `YYYY-MM-DD`. Penjelasan dari format tersebut dapat dilihat di sub bab Format Tanggal dan Waktu.  Contoh: `2022-06-28`. |
...
Bila resource Specimen dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource Specimen yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Specimen/5edd0663-093f-40f9-bf04-0c103fd6ec32",
      "resource": {
        "resourceType": "Specimen",
        "id": "5edd0663-093f-40f9-bf04-0c103fd6ec32",
        //data.terkait.resource.Specimen
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Specimen?subject=100000000001"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource Specimen yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource Specimen tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Specimen/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Specimen yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Specimen/5edd0663-093f-40f9-bf04-0c103fd6ec32"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Specimen yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource Specimen ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource Specimen ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *Specimen* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource Specimen ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource Specimen yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Specimen yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource Specimen yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Specimen yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* Specimen dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* Specimen dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* Specimen ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `Specimen.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
