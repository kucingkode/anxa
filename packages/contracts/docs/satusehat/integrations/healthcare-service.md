> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/healthcare-service/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource HealthcareService yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/HealthcareService
```
...
##### 1. Pencarian Berdasarkan Kode Spesialisasi/Keahlian

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `specialty` | `string` | Berisi kode dari spesialisasi/keahlian yang akan dicari.  Contoh: `S001.09`. |
...
Bila resource HealthcareService dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource HealthcareService yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/HealthcareService/16e8ab09-0c07-4486-ad7e-b708e6fafb2a",
      "resource": {
        "resourceType": "HealthcareService",
        "id": "16e8ab09-0c07-4486-ad7e-b708e6fafb2a",
        //data.terkait.resource.HealthcareService
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/HealthcareService?speciality=S001.09"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource HealthcareService yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource HealthcareService tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/HealthcareService/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource HealthcareService yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/HealthcareService/16e8ab09-0c07-4486-ad7e-b708e6fafb2a"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource HealthcareService yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource HealthcareService ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource HealthcareService ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *HealthcareService* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource HealthcareService ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource HealthcareService yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource HealthcareService yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource HealthcareService yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource HealthcareService yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource HealthcareService ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource HealthcareService yang akan diubah dan juga nama/ID elemen dari HealthcareService yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* HealthcareService dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* HealthcareService dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* HealthcareService ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `HealthcareService.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
