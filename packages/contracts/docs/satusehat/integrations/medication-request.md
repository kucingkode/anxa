> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/medication-request/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource MedicationRequest yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
Bila resource MedicationRequest dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource MedicationRequest yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/MedicationRequest/cf92db3e-a044-4e15-83fb-b7ec3a30ba76",
      "resource": {
        "resourceType": "MedicationRequest",
        "id": "cf92db3e-a044-4e15-83fb-b7ec3a30ba76",
        //data.terkait.resource.MedicationRequest
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/MedicationRequest?subject=100000000001"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource MedicationRequest yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource MedicationRequest tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/MedicationRequest/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource MedicationRequest yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/MedicationRequest/cf92db3e-a044-4e15-83fb-b7ec3a30ba76"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource MedicationRequest yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource MedicationRequest ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource MedicationRequest ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *MedicationRequest* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource MedicationRequest ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource MedicationRequest yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource MedicationRequest yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource MedicationRequest yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource MedicationRequest yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource MedicationRequest ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource MedicationRequest yang akan diubah dan juga nama/ID elemen dari MedicationRequest yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* MedicationRequest dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* MedicationRequest dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* MedicationRequest ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `MedicationRequest.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
