> Sumber asli: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/immunization/
...
Fungsi dari ReST API ini adalah untuk mencari data terkait resource Immunization yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Immunization
```
...
##### 1. Pencarian Berdasarkan ID Pasien dan/atau Tanggal Imunisasi

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `patient` | `string` | Berisi ID dari pasien yang akan dicari.  Contoh: `100000000001`. |
| `date` | `date` | Berisi tanggal dengan format `YYYY-MM-DD`. Penjelasan dari format tersebut dapat dilihat di sub bab Format Tanggal dan Waktu.  Contoh: `2022-01-11`. |
...
Bila resource Immunization dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource Immunization yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Immunization/cb9d1ff3-5c6c-4318-8ba7-14f800c4f8b0",
      "resource": {
        "resourceType": "Immunization",
        "id": "cb9d1ff3-5c6c-4318-8ba7-14f800c4f8b0",
        //data.terkait.resource.Immunization
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Immunization?subject={qs-immunization-subject}"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource Immunization yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource Immunization tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Immunization/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Immunization yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Immunization/cb9d1ff3-5c6c-4318-8ba7-14f800c4f8b0"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Immunization yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource Immunization ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource Immunization ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *Immunization* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource Immunization ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource Immunization yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Immunization yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource Immunization yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Immunization yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource Immunization ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource Immunization yang akan diubah dan juga nama/ID elemen dari Immunization yang akan dilakukan perubahan.
...
* `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* Immunization dengan ID terkait.
* `<element-path>`, nama properti/element dari *resource* Immunization dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* Immunization ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `Immunization.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
